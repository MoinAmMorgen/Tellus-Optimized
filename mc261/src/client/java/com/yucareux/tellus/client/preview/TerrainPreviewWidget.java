...
public final class TerrainPreviewWidget extends AbstractWidget implements AutoCloseable {
   ...
   private void renderBlurredBackground(GuiGraphicsExtractor graphics) {
      int left = this.getX();
      int top = this.getY();
      ...
      graphics.outline(left, top, this.width, this.height, 402653184);
   }

   private void renderLoadingOverlay(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
      if (this.preview.isLoading()) {
         ...
         if (status.stage() != TerrainPreview.PreviewStage.COMPLETE) {
            ...
            graphics.outline(panelX, panelY, panelWidth, panelHeight, LOADING_PANEL_BORDER);
            int textY = panelY + padding;
            graphics.text(font, label, panelX + padding, textY, LOADING_TEXT, true);
            graphics.text(font, percentText, panelX + panelWidth - padding - font.width(percentText), textY, LOADING_TEXT, true);
            ...
            graphics.outline(barX, barY, innerWidth, barHeight, LOADING_BAR_BORDER);
   ...
   private void renderInfoButton(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
      int buttonX = this.getX() + MODE_BUTTON_PADDING;
      ...
      this.infoButton.extractRenderState(graphics, mouseX, mouseY, delta);
   }

   private void renderInfoPanel(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
      ...
      graphics.outline(layout.x(), layout.y(), layout.width(), layout.height(), INFO_PANEL_BORDER);
      ...
      if (info == null) {
         ...
         graphics.text(font, this.truncate(font, demSummary, layout.contentWidth()), textX, textY, INFO_TEXT, false);
         textY += INFO_LINE_SPACING;
         String resolutionSummary = this.resolutionSummary(info);
         graphics.text(font, this.truncate(font, resolutionSummary, layout.contentWidth()), textX, textY, INFO_SUBTLE, false);
         textY += INFO_LINE_SPACING;
         if (info.hasBlendedProviders()) {
            ...
            graphics.text(font, this.truncate(font, blended, layout.contentWidth()), textX, textY, INFO_SUBTLE, false);
            textY += INFO_LINE_SPACING;
         }
         ...
         graphics.text(font, this.truncate(font, altitude, layout.contentWidth()), textX, textY, INFO_TEXT, false);
         textY += INFO_TITLE_SPACING;
         this.drawValueLine(
            graphics,
            font,
   ...
   private void drawValueLine(GuiGraphicsExtractor graphics, Font font, int x, int y, String label, String value, int valueColor, int width) {
      graphics.text(font, label, x, y, INFO_TEXT, false);
