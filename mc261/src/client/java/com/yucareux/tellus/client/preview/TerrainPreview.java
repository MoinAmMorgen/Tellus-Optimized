...
public final class TerrainPreview implements AutoCloseable {
      ...
      try {
         for (int z = 0; z < size; z++) {
            ...
            for (int x = 0; x < size; x++) {
               ...
               int pointCoverClass = this.landCoverSource.sampleCoverClass(blockX, blockZ, worldScale);
      ...
      for (int z = 0; z < size; z++) {
         ...
         for (int xxxx = 0; xxxx < size; xxxx++) {
            ...
            int color = colorForPreview(
               ...
               slope,
               seaLevel,
               settings.enableWater() && oceanFallbackMask[idx],
               settings.enableWater() && !waterPreviewEnabled,
      ...
      for (int z = 0; z < size; z++) {
         ...
         for (int x = 0; x < size; x++) {
            ...
            int color = colorForPreview(
               ...
               slope,
               seaLevel,
               settings.enableWater() && snapshot.oceanFallbackMask()[idx],
               settings.enableWater() && !waterPreviewEnabled,
               remaSnowEnabled && blockZ >= remaSnowBoundaryZ
            );
            terrainColors[idx] = color;
   ...
   private static int colorForPreview(
      ...
      double waterDepth = previewWaterDepthBlocks(surfaceCoverClass, terrainHeight, seaLevel, oceanWater, fallbackInlandWaterEnabled);
      if (waterDepth >= 0.0) {
   ...
   private static double previewWaterDepthBlocks(
      int coverClass, double terrainHeight, int seaLevel, boolean oceanWater, boolean fallbackInlandWaterEnabled
   ) {
      if (oceanWater) {
         return Math.max(0.0, seaLevel - terrainHeight);
      } else if (!fallbackInlandWaterEnabled) {
         return -1.0;
      } else if (coverClass == ESA_WATER) {
